...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	...
	<!-- 按钮 -->
	<div id="close" style="background: green; width: 10%; margin: 0 auto; height: 1.5rem; line-height: 1.5rem; text-align: center;color: #fff;margin-top: 28rem; -moz-border-radius: .128rem; -webkit-border-radius: .128rem; border-radius: .128rem;font-size: .59733rem;" onclick="closeWindow()">
		关闭
	</div>
</div>
...
<script type="text/javascript">
	...
	$(function(){
	    ...
	    $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/notice/getNewestNoticelist",
			success:function(msg){


                    $(msg.map.list).each(function(index,item){
                        var li = "<li class='ue-clear'>"
                          +" <span>"+ moment(item.ndate).format('YYYY-MM-DD')+"</span>&nbsp;&nbsp;&nbsp;<a onclick='show("+item.nid+")' href='javascript:void(0)'class='notice-title'>"+item.ntitle+"</a>"
                        +"</li>"
						$("#notice").append(li)
					})
	...
	function show(nid){
		...
		$.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/notice/getNoticeById/"+nid,
			success:function(msg){
        ...
        $("#showdiv").show();
        $("#cover").css({"height":document.body.scrollHeight }).show();
	}
